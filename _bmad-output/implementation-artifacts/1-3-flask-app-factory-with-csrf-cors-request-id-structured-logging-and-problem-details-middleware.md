...
- [x] Integration tests
  ...
  - [x] `tests/integration/test_csrf.py` — POST to a protected route without token → 400 with Problem Details; POST to `/api/v1/polls/<uuid>/submit` (stubbed) without token → 404 (not yet implemented) but NOT 400 (confirms exemption wiring)
