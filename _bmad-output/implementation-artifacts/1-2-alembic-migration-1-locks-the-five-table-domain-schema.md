...
- [ ] [Review][Patch] **Fix misleading test assertion message about "partial index"** [kano-backend/tests/integration/test_alembic_roundtrip.py:39-41]
- [ ] [Review][Patch] **CHECK constraint bare names in `models/response.py` should use `op.f()` style or omit `name=`** — works today by accident through the convention's `%(constraint_name)s` slot; future autogenerate may treat the model-side names as drift. [kano-backend/src/kano/models/response.py:36-41]
- [ ] [Review][Patch] **Replace `alembic.ini` placeholder URL with a fail-fast value** — current `postgresql+psycopg2://user:pass@localhost/dbname` is overridden by env.py but lands in the repo, fools casual credential greps, and would become the live default if the env.py guard is ever removed. Use `driver://CONFIGURE_VIA_ENV_DATABASE_URL` (parses-but-never-connects). [kano-backend/alembic.ini:90]
- [ ] [Review][Patch] **Add CHECK boundary tests** — `fq_answer=0`, `dq_answer=0`, `category=''`, `category='m'` (lowercase). Current test only exercises 6 (above range) and 'X' (not in enum). [kano-backend/tests/integration/test_alembic_roundtrip.py:120-140]
- [ ] [Review][Patch] **Make `CREATE INDEX` / `DROP INDEX` idempotent** — `op.execute("CREATE INDEX ix_polls_expires_at …")` lacks `IF NOT EXISTS`; the matching DROP has `IF EXISTS` but the `op.drop_index` calls in `downgrade()` do not. A partial-failure mid-upgrade leaves the DB in a state where downgrade fails. [migrations/versions/0001_initial_schema.py:125, 188-197]
- [ ] [Review][Patch] **Update `EXPECTED_TIMESTAMP_COLUMNS` docstring/comment** — comment says "every `*_at` column across all 5 tables"; `responses` has no timestamp columns by design. Either drop the "5 tables" claim or note the responses exclusion. [kano-backend/tests/integration/test_timestamptz.py:14-22]
