...
Adversarial review pass (resolved 2026-05-11). Manual smoke remains the acceptance bar for this story per user direction; CI-runtime gaps (compose-config validation in CI, KANO_VERSION boot smoke) are owned by Story 1.10 and not addressed here.

- [x] [Review][Patch] `.env.example` carried a separate `DATABASE_URL` line that compose then re-interpolated from `POSTGRES_*` — two values to keep in sync, guaranteed drift on credential rotation [`/.env.example`]. **Resolved:** dropped the redundant line and added a header block calling out the single source of truth (POSTGRES_*) for the compose workflow plus a pointer to README's "Without Docker" recipe for the non-compose path. `kano-backend/config.py` still reads `DATABASE_URL` from the environment for non-compose runs; nothing breaks.
- [x] [Review][Patch] `entrypoint.sh` had no wait-for-db retry — relied entirely on compose's `depends_on: condition: service_healthy`, which the runbook itself admitted could race [`kano-backend/entrypoint.sh`]. **Resolved:** wrapped `alembic upgrade head` in a bounded retry loop (15× × 2 s by default, both configurable via `ALEMBIC_RETRY_ATTEMPTS` / `ALEMBIC_RETRY_SLEEP` env vars). The "restart the container manually" failure mode is gone.
- [x] [Review][Patch] `POSTGRES_DB` change after first boot fails opaquely (Postgres image creates the DB only on init; subsequent boots reuse the named volume) [`docs/ops/runbook.md`]. **Resolved:** added an explicit troubleshooting section showing the failure message + the `docker compose down -v` reset recipe with a backup pre-step.
- [x] [Review][Patch] `db` exposed `5432:5432` on all interfaces — placeholder credentials on the LAN [`docker-compose.yml`]. **Resolved:** bound to `127.0.0.1:5432:5432` (loopback only); inline comment documents how to flip it temporarily if needed and points at the `exec db psql` recipe for the routine case.
- [x] [Review][Patch] `api` host port `5000:5000` collides with macOS AirPlay Receiver — the implementer's own debug log mentioned a similar collision [`docs/ops/runbook.md`]. **Resolved:** added a Troubleshooting entry showing both the diagnostic command (`sudo lsof -i :5000`) and the workaround (remap host port to 5001).
- [x] [Review][Patch] `web` bind-mount list was hand-maintained and already missing files added by stories 1-7 / 1-8 (`eslint.config.js`, `playwright.config.ts`, `vitest.config.ts`) [`docker-compose.yml`]. **Resolved:** added the three missing mounts plus `tests/` and `e2e/` directories so unit/E2E specs run inside the container; updated the inline comment so future contributors know the list is hand-maintained.
- [x] [Review][Patch] Production Caddyfile had no cache or security headers [`kano-frontend/Caddyfile`]. **Resolved:** added `Cache-Control: public, max-age=31536000, immutable` for hashed assets + fonts; `no-cache, no-store, must-revalidate` on `index.html` and `/`; security headers `X-Content-Type-Options nosniff`, `X-Frame-Options DENY`, `Referrer-Policy strict-origin-when-cross-origin`; dropped the `Server` banner header. Documented the HTTPS gap as owned by Story 7-1 (the `:80` listener becomes a domain-bound TLS block alongside the public domain definition).
- [x] [Review][Patch] `web` service had no healthcheck — Story 1.10's `docker compose up -d --wait` would have returned the moment Vite's container started, not when Vite was actually serving [`kano-frontend/Dockerfile` dev stage]. **Resolved:** added `HEALTHCHECK CMD wget --quiet --spider http://localhost:5173/` to the dev stage with a 45-s start-period covering Vite's cold-start.
- [x] [Review][Patch] `docker-compose.yml` had no top-level `name:`, so volume names varied with the clone directory [`docker-compose.yml`]. **Resolved:** pinned `name: kano`; runbook `docker volume ls | grep kano` commands now work regardless of clone location.
- [x] [Review][Patch] No resource limits on any container [`docker-compose.yml`]. **Resolved:** modest `mem_limit` on db (1 g), api (1 g), web (2 g) — enough headroom for the realistic dev workload, low enough to catch a runaway before it OOMs a laptop.
- [x] [Review][Patch] `backup` service `depends_on: db: condition: service_healthy` was decorative for a `sleep infinity` placeholder [`docker-compose.yml`]. **Resolved:** removed the dependency with a comment indicating Story 6-1 will reintroduce it when real `pg_dump` cron logic lands; backup boot critical path is now sub-second.
- [x] [Review][Patch] `api` HEALTHCHECK installed `curl` + an `apt-get update` layer just to read one URL [`kano-backend/Dockerfile`]. **Resolved:** dropped the `apt-get update && install curl && rm -rf /var/lib/apt/lists/*` triplet; HEALTHCHECK now uses `python -c "import urllib.request; ..."` which uses only the stdlib. Image is slightly smaller and the supply-chain surface shrinks by one system package.
- [x] [Review][Patch] Runbook embedded Story 1.10's CI table verbatim — drift the moment 1.10 changes any job name [`docs/ops/runbook.md`]. **Resolved:** replaced the embedded table with a one-paragraph stub pointing at Story 1.10 as the owner; the real config will live in `.github/workflows/ci.yml` and `.pre-commit-config.yaml`.
- [x] [Review][Patch] Runbook's `docker compose down -v` recipe (for the EACCES fix) destroyed all volumes including db_data [`docs/ops/runbook.md`]. **Resolved:** scoped the EACCES fix to `docker volume rm kano_frontend_node_modules` only, and added an explicit "destroys all DB data" warning at the top of the smoke-checklist `down -v` step with a `pg_dump` pre-step recipe.
- [x] [Review][Patch] Runbook claimed "~30 s" first-boot time [`docs/ops/runbook.md`]. **Resolved:** corrected to "1-3 minutes" with the npm-ci-dominated explanation; subsequent boots stay sub-second because of the named volume.
- [x] [Review][Patch] README's "Without Docker" section was a 4-line trap (poetry install + npm install with no DB, no env vars, no migrations) [`README.md`]. **Resolved:** rewrote with the full recipe — standalone Postgres container, env-var exports including DATABASE_URL, alembic upgrade, the right flask command, a parallel `npm run dev` step, and a smoke `curl` — plus an honest disclaimer that CI doesn't exercise this path.
- [x] [Review][Patch] Story status `review` with zero Review Findings section drafted — sixth story in a row. **Resolved:** this section.
...
| 2026-05-11 | 0.2.0   | Adversarial-review fix-up. **Config drift killed:** removed redundant `DATABASE_URL` from `.env.example` (compose interpolates from `POSTGRES_*`; non-Docker path documented in README). **Boot races eliminated:** `entrypoint.sh` retries `alembic upgrade head` up to 15× × 2 s before giving up; the "restart the container manually" runbook recipe is gone. **Production Caddyfile hardened:** added `Cache-Control` (immutable for hashed assets/fonts; no-cache for `index.html`/`/`) plus `X-Content-Type-Options`, `X-Frame-Options DENY`, `Referrer-Policy strict-origin-when-cross-origin`, and dropped the `Server` banner; HTTPS deferred to Story 7-1 (documented). **Compose hardened:** `name: kano` for deterministic volume names; `db` bound to `127.0.0.1` only; `mem_limit` on db/api/web; decorative `depends_on` removed from the `backup` placeholder. **Healthchecks:** `web` dev stage now has a `wget --spider` healthcheck so Story 1.10's `--wait` gates on actual readiness; api HEALTHCHECK switched from `curl` (extra system package + `apt-get update` layer) to `python -c "import urllib.request"` (stdlib only, smaller image, smaller supply chain). **Bind mounts repaired:** `web` now mounts `eslint.config.js`, `playwright.config.ts`, `vitest.config.ts`, `tests/`, `e2e/` (missed by the original story; added by 1-7 / 1-8). **Runbook fixes:** macOS port-5000 collision + `POSTGRES_DB`-change footgun documented with concrete recipes; EACCES fix scoped to `kano_frontend_node_modules` only (was `down -v` and destroying DB data); `down -v` step warns about data loss with a `pg_dump` pre-step; the embedded Story 1.10 CI table replaced with an "owned by 1.10" stub. **README:** the trap-style 4-line "Without Docker" section rewritten with a complete recipe (standalone Postgres container, env-var exports, alembic upgrade, parallel `npm run dev`, smoke `curl`) plus a disclaimer that CI doesn't exercise this path. **Gates:** backend 70/70 pytest green; frontend 98/98 vitest green; type-check + build clean; `docker compose config` validates against `.env.example`. Manual `docker compose up` re-verification is the dev's pre-merge step (per user direction). | Jolan (review fix-up) |
